-- FINAL DATABASE SETUP - Run this entire script
...
-- Drop all tables first (this will automatically drop policies)
...
-- Set up your account as teacher
...
((SELECT id FROM auth.users WHERE email = 'pratiksindhiya3@gmail.com'), 'Charlie Wilson', 'Physics', 95);
...
SELECT 'RLS Status:' as info, tablename, rowsecurity FROM pg_tables 
WHERE schemaname = 'public' AND tablename IN ('students', 'teachers', 'user_roles');

-- Check policies exist
SELECT 'Policies Created:' as info, COUNT(*) as total_policies FROM pg_policies 
WHERE tablename IN ('students', 'teachers', 'user_roles');

-- Check your setup
SELECT 'Your Account:' as info, ur.role, t.name, u.email
FROM user_roles ur
